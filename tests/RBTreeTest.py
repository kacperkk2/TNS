...
def two_elements_in_tree():
    ...
    assert tree.root.left.rule is None
    print(tree)
    ...
    tree.add(rule2)
    assert tree.root.rule == rule1
    assert tree.root.right.rule is None
    assert tree.root.left.rule == rule2
    print(tree)


def multiple_elements_in_tree_delete_max_and_min():
    tree = RBTree()
    rule1 = Rule([1, 2, 3], [5, 6, 7], 1.0, 0, None, None, None, None, None)
    rule2 = Rule([1, 2, 3], [5, 6], 0, 0, None, None, None, None, None)
    ...
    poped_rule = tree.pop_minimum()
    assert poped_rule == rule4
    assert tree.size == 3

    poped_rule = tree.pop_maximum()
    assert poped_rule == rule3
    assert tree.size == 2
    print(tree)


def multiple_elements_in_tree_delete_root():
    tree = RBTree()
    rule1 = Rule([1, 2, 3], [5, 6, 7], 1.0, 0, None, None, None, None, None)
    rule2 = Rule([1, 2, 3], [5, 6], 0, 0, None, None, None, None, None)
    rule3 = Rule([1, 2, 3, 4, 5], [5], 0, 0, None, None, None, None, None)
    ...
    tree.remove(tree.root.rule)
    assert tree.root.rule == rule2
    assert tree.root.left.rule == rule4
    assert tree.root.right.rule == rule3
    print(tree)
...
if __name__ == '__main__':
    ...
    multiple_elements_in_tree_delete_max_and_min()
    multiple_elements_in_tree_delete_root()
